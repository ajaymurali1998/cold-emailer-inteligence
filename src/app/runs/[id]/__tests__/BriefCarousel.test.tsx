...
describe('BriefCarousel', () => {
  it.each(USER_INTENTS)('renders correctly for %s without type errors', (intent) => {
    ...
    render(<BriefCarousel runId="run-1" briefs={[brief]} emailDraft={null} />)

    expect(screen.getByDisplayValue(brief.headline)).toBeTruthy()
    for (const label of labels) {
      expect(screen.getAllByText(label).length).toBeGreaterThan(0)
    }
  })

  it('renders the final email draft instead of the editor once one exists', () => {
    const brief: StrategyBriefSnapshot = {
      id: 'brief-1',
      headline: 'Headline',
      coreInsights: [],
